...
namespace SCG.ARS.BOI.WEB.Controllers
{
    ...
    {
        [HttpPost]
        public async Task<JsonResult> NotAcceptedStatus(TransportationCriteria criteria)
        {
            await FilterCriteria(criteria);
            try
            {
                
                //return Json(new { data = _report.NotAcceptedStatus(criteria), success = true });
                List<TransportationStatusRawModel> rawdata = _report.NotAcceptedStatus(criteria);
            ...
            {
                logger.Error(ex, "NotAcceptedStatus", Newtonsoft.Json.JsonConvert.SerializeObject(criteria));
                return Json(new { data = ex.Message, success = false });
            }
        }

        [HttpPost]
        public async Task<JsonResult> NotAcceptedByBusiness(TransportationCriteria criteria)
        {
            await FilterCriteria(criteria);
            try
            {
                List<TransportationByBusinessModel> rawdata = _report.NotAcceptedByBusiness(criteria);
            ...
            {
                logger.Error(ex, "NotAccepted", Newtonsoft.Json.JsonConvert.SerializeObject(criteria));
                return Json(new { data = ex.Message, success = false });
            }
        }

        [HttpPost]
        public async Task<JsonResult> NotAcceptedByCarrier(TransportationCriteria criteria)
        {
            await FilterCriteria(criteria);
            try
            {
                List<TransportationNotAcceptedByCarrierModel> rawdata = _report.NotAcceptedByCarrier(criteria);
        ...
        [HttpPost]
        public async Task<JsonResult> NotAcceptedDetail(TransportationCriteria criteria)
        {
            await FilterCriteria(criteria);
            try
            {
                return Json(new { data = _report.NotAcceptedDetail(criteria), success = true });
            }
            catch (Exception ex)
            {
                logger.Error(ex, "NotAcceptedDetail", Newtonsoft.Json.JsonConvert.SerializeObject(criteria));
                return Json(new { data = ex.Message, success = false });
            }
        }
