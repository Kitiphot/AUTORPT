$(function () {
	...
	$(".top-bar .hamburg-icon").on("click", function (e) {
		...
		if ($("#wrapper").hasClass('small-nav')) {
			$('#left-menu').find('.nav-link[data-toggle="tooltip"]').tooltip();
			$('#left-menu').find('.navbar').removeClass('hide-scroll-x');
			$('#left-menu').find('.level1').each((idx, elem) => {
				if ($(elem).find('.has-submenu').length > 0) {
					$(elem).addClass('override-level1');
				}
			});
			//$('.level1').addClass('override-level1');
		}
		else {
			$('#left-menu').find('.nav-link[data-toggle="tooltip"]').tooltip('dispose');
			$('#left-menu').find('.nav-link[data-toggle="tooltip"]').each((idx, elem) => {
				$(elem).attr('title', $(elem).attr('data-original-title'))
			})
			$('#left-menu').find('.navbar').addClass('hide-scroll-x');
			$('#left-menu').find('.level1').removeClass('override-level1');
		}
	});
	...
	$(".nav-item .nav-link").on("click", function (e) {
		if ($(this).hasClass("has-submenu")) {
			...
			$('#left-menu').find('.nav-link[data-toggle="tooltip"]').tooltip('hide');
		}
	});
