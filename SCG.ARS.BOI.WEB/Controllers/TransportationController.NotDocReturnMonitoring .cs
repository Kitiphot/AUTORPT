...
namespace SCG.ARS.BOI.WEB.Controllers
{
    ...
    {
        [HttpPost]
        public async Task<JsonResult> DocReturnStatus(TransportationCriteria criteria)
        {
            await FilterCriteria(criteria);
            try
            {
                List<TransportationDocReturnStatusMonitorModel> rawdata = _report.DocReturnStatus(criteria);
            ...
            {
                logger.Error(ex, "DocReturnStatus", Newtonsoft.Json.JsonConvert.SerializeObject(criteria));
                return Json(new { data = ex.Message, success = false });
            }
        }

        [HttpPost]
        public async Task<JsonResult> DocReturnByBusiness(TransportationCriteria criteria)
        {
            await FilterCriteria(criteria);
            try
            {
                List<TransportationByBusinessModel> rawdata = _report.DocReturnByBusiness(criteria);
            ...
            {
                logger.Error(ex, "DocReturn", Newtonsoft.Json.JsonConvert.SerializeObject(criteria));
                return Json(new { data = ex.Message, success = false });
            }
        }

        [HttpPost]
        public async Task<JsonResult> DocReturnByCarrier(TransportationCriteria criteria)
        {
            await FilterCriteria(criteria);
            try
            {
                List<TransportationByCarrierMonitoringModel> rawdata = _report.DocReturnByCarrier(criteria);
        ...
        [HttpPost]
        public async Task<JsonResult> DocReturnDetail(TransportationCriteria criteria)
        {
            await FilterCriteria(criteria);
            try
            {
                return Json(new { data = _report.DocReturnDetail(criteria), success = true });
            }
            catch (Exception ex)
            {
                logger.Error(ex, "DocReturnDetail", Newtonsoft.Json.JsonConvert.SerializeObject(criteria));
                return Json(new { data = ex.Message, success = false });
            }
        }
