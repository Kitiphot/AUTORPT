...
namespace SCG.ARS.BOI.WEB.Services {
    ...
    public class PbiEmbedService
    {
        ...
        public PbiEmbedService(IOptions<PowerBISetting> setting) {
        ...
        public PowerBIClient GetPowerBIClient()
        {
            ...
            var tokenCredentials = new TokenCredentials(GetAccessTokenTemp(), "Bearer");
            return new PowerBIClient(new Uri(urlPowerBiServiceApiRoot ), tokenCredentials);
        }

        private string GetAccessTokenTemp() {
            var request = (HttpWebRequest)WebRequest.Create("https://scgl-powerbiembedded.azurewebsites.net/api/token");
            var postData = $"{{Username: \"{setting.UserName}\", Password: \"{setting.Password}\"}}";
        ...
        public PowerBIEmbedConfigModel GetEmbedParams(Guid workspaceId, Guid reportId, [Optional] Guid additionalDatasetId)
        {
            PowerBIClient pbiClient = this.GetPowerBIClient();

            // Get report info
            var pbiReport = pbiClient.Reports.GetReportInGroup(workspaceId.ToString(), reportId.ToString());
            ...
            var embedParams = new PowerBIEmbedConfigModel {
                Id = pbiReport.Id,
        ...
        public EmbedToken GetEmbedToken(Guid reportId, IList<Guid> datasetIds, [Optional] Guid targetWorkspaceId)
        {
            ...
            var tokenRequest = new GenerateTokenRequestV2(
                ...
                targetWorkspaces: targetWorkspaceId != Guid.Empty ? new List<GenerateTokenRequestV2TargetWorkspace>() { new GenerateTokenRequestV2TargetWorkspace(targetWorkspaceId.ToString()) } : null
            );
        ...
        public EmbedToken GetEmbedToken(IList<Guid> reportIds, IList<Guid> datasetIds, [Optional] Guid targetWorkspaceId)
        {
            ...
            var reports = reportIds.Select(reportId => new GenerateTokenRequestV2Report(reportId.ToString())).ToList();
        ...
        public EmbedToken GetEmbedToken(IList<Guid> reportIds, IList<Guid> datasetIds, [Optional] IList<Guid> targetWorkspaceIds)
        {
            ...
            {
                targetWorkspaces = targetWorkspaceIds.Select(targetWorkspaceId => new GenerateTokenRequestV2TargetWorkspace(targetWorkspaceId.ToString())).ToList();
            }
